...
func (p *printer) marshalValue(val reflect.Value, finfo *fieldInfo, startTemplate *StartElement) error {
	...
	if finfo != nil && finfo.selfclose {
		...
		for i := range tinfo.fields {
			...
			fv := finfo.value(val)
			if finfo.flags&fCharData != 0 && isEmptyValue(fv) {
				continue
			}
			if fv.Kind() == reflect.Interface && fv.IsNil() {
				continue
	...
	if err := p.writeStart(&start, selfclose); err != nil {
		return err
	}
...
func (p *printer) writeStart(start *StartElement, selfClose bool) error {
	if start.Name.Local == "" {
...
func (p *printer) marshalSimple(typ reflect.Type, val reflect.Value) (string, []byte, error) {
	...
	case reflect.Float32, reflect.Float64:
		return strconv.FormatFloat(val.Float(), 'f', -1, val.Type().Bits()), nil, nil
	case reflect.String:
